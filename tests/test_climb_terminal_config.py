...
class ClimbTerminalConfigTest(unittest.TestCase):
    def test_source_motion_ends_without_an_extra_terminal_hold(self):
        ...
        self.assertEqual(
            ast.literal_eval(assignments["ELF3_CLIMB_RANDOM_PHASE_ENV_FRACTION"]),
            1.0,
        )
        ...
        for reward_name in (
            "final_expert_joint_pose",
            "final_actual_joint_velocity",
            "final_ankle_surface_settling",
            "final_expert_root_orientation",
            "final_expert_root_linear_velocity",
            "final_expert_root_angular_velocity",
        ):
            with self.subTest(removed_terminal_reward=reward_name):
                self.assertNotIn(reward_name, reward_names)
        for reward_name in (
        ...
        self.assertIsInstance(curriculum_assignment.value, ast.Constant)
        self.assertIsNone(curriculum_assignment.value.value)

        events_class = next(
            node for node in tree.body if isinstance(node, ast.ClassDef) and node.name == "ELF3ClimbEventCfg"
        )
        platform_pose_assignment = next(
            node
            for node in events_class.body
            if isinstance(node, ast.Assign)
            and any(isinstance(target, ast.Name) and target.id == "platform_pose" for target in node.targets)
        )
        platform_pose_keywords = {
            keyword.arg: keyword.value for keyword in platform_pose_assignment.value.keywords
        }
        platform_pose_params = {
            ast.literal_eval(key): value
            for key, value in zip(
                platform_pose_keywords["params"].keys,
                platform_pose_keywords["params"].values,
                strict=True,
            )
        }
        self.assertEqual(ast.literal_eval(platform_pose_params["position_range"]), {"x": (0.0, 0.0), "y": (0.0, 0.0)})
        self.assertEqual(ast.literal_eval(platform_pose_params["yaw_range"]), (0.0, 0.0))
